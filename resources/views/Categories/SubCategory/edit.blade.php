                ...
                <div class="row">
                    <div class="col-xs-12">
                        <form class="form-horizontal" method="POST"
                              action="{{route('category.sub.update',['id'=>$obj->id])}}">
                            @csrf
                            <div class="row">
                                <div class="col-md-6 mt-2">
                                    ...
                                    <select class="form-control" name="parentCategoryName" required>
                                        ...
                                        @foreach ( $ParentCategories as $ParentCategory )
                                            <option
                                                value="{{$ParentCategory->id}}"{{$ParentCategory->id==$obj->parentCategoryId?'selected':''}}>{{$ParentCategory->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 mt-2">
                                    <label><b>Enter SubCategory Name</b></label>
                                    <input type="text" name="name" placeholder="Category Name" value="{{$obj->name}}"
